...
// DRIFT DIFFUSION MODEL: BAYESIAN w/BIASED PRIOR & BIASED DRIFT
...
model {
    ...
    //print ("threshold_int: ", threshold_int, " fixation_bias: ", fixation_bias, " cong_weight: ", cong_weight, " drift_rate_learning: ", drift_rate_learning, "non_decision_time_int:", non_decision_time_int);
	...
	for (s in 1:NS) {
		...
		for (p in 1:NP) {
			...
			cf = 0;
			bid_cong_prior[1] = cong_weight_prior[s] * bid_congruence[s,p,1] ;
			bid_cong_prior[2] = 0;
			bid_cong_drift[1] = cong_weight_drift_bias[s] * bid_congruence[s,p,1];
			bid_cong_drift[2] = 0;
			...
			q[2] = softmax(bid_cong_prior)[1]; // perceived probability of CORRECT answer being correct
