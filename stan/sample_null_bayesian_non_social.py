##################################################
### DRIFT DIFFUSION MODEL: BAYESIAN NULL (NON-SOCIAL GROUP) ###
##################################################
...
# Import data
data = pd.read_pickle(r'../data/control-data-proc-pkl.pkl')
...
model_code_obj = pystan.StanModel(file='stan/model_null_bayesian_non_social.stan.cpp', model_name='model_null_bayesian_non_social') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('stan/pickles/model_null_bayesian_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('stan/pickles/fit_null_bayesian_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
