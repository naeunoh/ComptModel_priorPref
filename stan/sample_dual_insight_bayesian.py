...
# Import data
data = pd.read_pickle(r'../data/social/data_processed_social_wexclusions_wpopularity.pkl')
...
model_data = {'NS': 31, 'NP': 20, 'NT': 30, 'correct': model_data_correct, 'feedback': model_data_feedback, 'bid_congruence': model_data_bid_congruence, 'rt': model_data_rt, 'rt_mins': model_rt_mins, 'item_popularity': model_data_item_popularity}

# Estimate models w/ pystan
# MODEL 2: dual inference model w/ popularity prior
model_code_obj = pystan.StanModel(file='stan/model_dual_insight_prior_bayesian.stan.cpp', model_name='model_dual_insight_prior_bayesian') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('stan/pickles/model_dual_insight_prior_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('stan/pickles/fit_dual_insight_prior_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)

print(fit)

# MODEL 3: dual inference model w/ popularity bias
model_code_obj = pystan.StanModel(file='stan/model_dual_insight_bias_bayesian.stan.cpp', model_name='model_dual_insight_bias_bayesian') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('stan/pickles/model_dual_insight_bias_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('stan/pickles/fit_dual_insight_bias_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)

print(fit)

# MODEL 4: dual inference model w/ popularity prior & bias
model_code_obj = pystan.StanModel(file='stan/model_dual_insight_priorbias_bayesian.stan.cpp', model_name='model_dual_insight_priorbias_bayesian') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('stan/pickles/model_dual_insight_priorbias_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('stan/pickles/fit_dual_insight_priorbias_bayesian.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
