##################################################
### DRIFT DIFFUSION MODEL: BAYESIAN w/BIASED PRIOR & CHOICE PROCESS (NON-SOCIAL GROUP) ###
##################################################
...
# Import data
data = pd.read_pickle(r'../data/non_social/data_processed_non_social_wexclusions.pkl')
...
# Replace subject C12, C26, and C31's implausibly low minimum RT with the lowest RT from all other subjects' data
model_rt_mins[3] = 0.41 
...
model_code_obj = pystan.StanModel(file='stan/model_dual_bayesian_non_social.stan.cpp', model_name='model_dual_bayesian_non_social') # Specific to model
fit = model_code_obj.sampling(data=model_data, iter=2000, chains=4, refresh=10)

with open('stan/pickles/model_dual_bayesian_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(model_code_obj, f)

with open('stan/pickles/fit_dual_bayesian_non_social.pkl', 'wb') as f: # Specific to model
    pickle.dump(fit, f)
