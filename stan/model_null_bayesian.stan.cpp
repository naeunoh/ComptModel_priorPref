...
model {
	...
	for (s in 1:NS) {
		...
		for (p in 1:NP) {
			...
			cf = 0;
			pri = 0.5; // prior is unbiased
			b = 0.5; // drift bias term is unbiased
...
generated quantities {
    ...
    // print(" ");
    // print("cong_weight_drift_bias_mean: ", cong_weight_drift_bias_mean);
    // print("cong_weight_drift_bias_sd: ", cong_weight_drift_bias_sd);
    // print("threshold_int_mean: ", threshold_int_mean);
    // print("threshold_int_sd: ", threshold_int_sd);
    // print("drift_rate_learning_mean: ", drift_rate_learning_mean);
    // print("drift_rate_learning_sd: ", drift_rate_learning_sd);
    // print("fixation_bias_mean: ", fixation_bias_mean);
    // print("fixation_bias_sd: ", fixation_bias_sd);
		...
		for (p in 1:NP) {
			...
			for (t in 1:NT) {
				...
				if (correct[s,p,t] == 1) {
					...
					if (rt[s,p,t] > -1) {
						log_lik_rt[ix] = wiener_lpdf(rt[s,p,t]| a, ti, 1-b, -v); // For incorrect responses, return lower boundary
					}
